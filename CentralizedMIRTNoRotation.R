...
MCrep <- 10 # number of Monte Carlo replications
...
truth <- cbind(a,d) # ground truth matrix
...
for(iter in 1:MCrep){
  ...
  values <- mirt(data,p, method = 'MHRM',TOL = 0.0001,
                 technical = list(MHDRAWS = 1,NCYCLES=1e5,gain = c(1,1)),pars = "values")
  ...
  model <- mirt(data, p, method = 'MHRM', itemtype = 'graded', 
                TOL = 0.0001,technical = list(NCYCLES=1e5,MHDRAWS = 1,gain = c(1,1)),pars = values)
  ...
  loadings <- QRDecomp(t(results[,1:2]))$R
  sinThetaDist[iter] = sinTheta(loadings, a)
...
print("Centralized MIRT without rotation")
