...
plot_fix_J_regime <- function(n) {
  ...
  mhrm_err_prod_f <- cjmle_err_prod_f <- rep(0, len_n)
  ...
  for (idx in 1:len_n) {
    ...
    cjmle_err_prod_f[idx] <- mean(rds["cjmle_err_prod_f"])
    ...
    mhrm_err_prod_f[idx] <- mean(rds["mhrm_err_prod_f"])
  }
  ic_green <- "#00b384"
  amber <- "#FFBF00"
  ...
  ggplot(NULL, aes(x = n)) +
    ...
    geom_line(aes(y = mhrm_err_prod_f, colour = "MHRM"), size = 1) +
    geom_point(aes(y = cjmle_err_prod_f), shape = 0, size = 2.5, color = ic_green) +
    geom_point(aes(y = mhrm_err_prod_f), shape = 0, size = 2.5, color = amber) +
    scale_color_manual(name = "Estimator", values = c("CJMLE" = ic_green, "MHRM" = amber)) +
    scale_x_continuous(breaks = n) +
...
plot_fix_N_regime <- function(j) {
  ...
  mhrm_err_prod_f <- cjmle_err_prod_f <- rep(0, len_j)
  # Extract data from existing .rds data
  for (idx in 1:len_j) {
    # print(n[idx])
    rds <- readRDS(paste("checkpoint/Fix-N[1000]/Fix-N[cc=2][J=", j[idx], "].rds", sep = ""))
    cjmle_A[idx] <- mean(rds["cjmle_err_A"])
    cjmle_d[idx] <- mean(rds["cjmle_err_d"])
    cjmle_theta[idx] <- mean(rds["cjmle_err_theta"])
    cjmle_prod[idx] <- mean(rds["cjmle_err_prod"])
  ...
  ggplot(NULL, aes(x = j)) +
    ...
    ggtitle("SinTheta Distance vs. Number of Items [A]") +
    easy_center_title()
  ggsave("checkpoint/Fix-N[1000]/plot/Fix-N[1000][A].png")
  ...
  ggplot(NULL, aes(x = j)) +
    ...
    ggtitle("F-Norm vs. Number of Items [d]") +
    easy_center_title()
  ggsave("checkpoint/Fix-N[1000]/plot/Fix-N[1000][d].png")
  ...
  ggplot(NULL, aes(x = j)) +
    ...
    ggtitle("SinTheta Distance vs. Number of Items [Theta]") +
    easy_center_title()
  ggsave("checkpoint/Fix-N[1000]/plot/Fix-N[1000][theta].png")
  ...
  ggplot(NULL, aes(x = j)) +
    ...
    ggtitle("Normalized F-norm vs. Sample Size [Product]") +
    easy_center_title()
  ggsave("checkpoint/Fix-N[1000]/plot/Fix-N[Product][F].png")
  ...
  ggplot(NULL, aes(x = j)) +
    ...
    scale_x_continuous(breaks = j) +
    labs(x = "Number of Items", y = "SinTheta Distance") +
    ggtitle("SinTheta Distance vs. Number of Items [Product]") +
    easy_center_title()
  ggsave("checkpoint/Fix-N[1000]/plot/Fix-N[1000][Product].png")
